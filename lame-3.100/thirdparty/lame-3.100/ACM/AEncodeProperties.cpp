	...
	if (my_stored_data.LoadFile(my_store_location))
	{
		...
		if (node != NULL)
		{
			...
			if (ConfigNode != NULL)
			{
				...
				while (tmpNode != NULL)
				{
					const std::string * tmpname = tmpNode->Attribute(std::string("name"));
...
void AEncodeProperties::GetValuesFromKey(const std::string & config_name, TiXmlNode & parentNode)
{
	TiXmlElement* tmpElt;
	TiXmlElement* iterateElmt;

	// find the config that correspond to CurrentConfig
	iterateElmt = parentNode.FirstChildElement(std::string("config"));
	while (iterateElmt != NULL)
	{
		const std::string * tmpname = iterateElmt->Attribute(std::string("name"));
	...
	if (iterateElmt != NULL)
	{
		...
		if (tmpElt != NULL)
		{
			tmpname = tmpElt->Attribute(std::string("use"));
			if (tmpname != NULL)
				bSmartOutput = (tmpname->compare("true") == 0);
			
			tmpname = tmpElt->Attribute(std::string("ratio"));
			if (tmpname != NULL)
		...
		if (tmpElt != NULL)
		{
			...
			tmpname = tmpElt->Attribute(std::string("min"));
			if (tmpname != NULL)
				AverageBitrate_Min = atoi(tmpname->c_str());

			tmpname = tmpElt->Attribute(std::string("max"));
			if (tmpname != NULL)
				AverageBitrate_Max = atoi(tmpname->c_str());

			tmpname = tmpElt->Attribute(std::string("step"));
			if (tmpname != NULL)
...
//#ifdef OLD
		...
		if (tmpElt != NULL)
		{
			const std::string * tmpStr = tmpElt->Attribute(std::string("mode"));
			if (tmpStr != NULL)
			{
...
bool AEncodeProperties::RenameCurrentTo(const std::string & new_config_name)
{
	...
	if (my_stored_data.LoadFile(my_store_location))
	{
		...
		if (CurrentNode->Attribute("default") != NULL)
		{
			std::string CurrentConfigName = *CurrentNode->Attribute(std::string("default"));
